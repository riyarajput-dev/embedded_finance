...
def load_config(config_path="kft_to_lender_4/config/config.yaml"):
    with open(config_path, 'r') as f:
        config_dict = yaml.safe_load(f)
    return ConfigBox(config_dict, default_box=True)

try:
    config_ktl = load_config()
except Exception as e:
    config_ktl = None  

logger_ktl = setup_logger(config_ktl)
...
def snowflake_connection_obj(database: str, schema: str) -> Session:
    logger_ktl.info("creating snowflake session.")
    ...
    logger_ktl.info(f"using database {database} and schema {schema}")
    logger_ktl.info("session created successfully.")
    return session
